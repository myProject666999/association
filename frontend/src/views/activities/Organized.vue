    ...
    <el-card class="common-card">
      ...
      <el-table :data="activityList" stripe style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="报名情况" width="120">
          <template #default="scope">
            {{ scope.row.current_participants || 0 }}/{{ scope.row.max_participants || '不限' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="scope">
            ...
            <el-button type="success" link size="small" @click="handleEdit(scope.row)" :disabled="scope.row.status === 1">
              编辑
            </el-button>
            <el-button 
              v-if="userStore.isAdmin() && scope.row.status === 0" 
              type="warning" 
              link 
              size="small" 
      ...
      <el-empty v-if="!loading && activityList.length === 0" description="暂无组织的活动" />
    </el-card>
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await api.get('/api/profile/organized-activities', { params })
    if (res.code === 200) {
...
const handleEdit = (activity) => {
  ...
  activityForm.start_time = activity.start_time ? new Date(activity.start_time) : null
  activityForm.end_time = activity.end_time ? new Date(activity.end_time) : null
  activityForm.registration_start = activity.registration_start ? new Date(activity.registration_start) : null
  activityForm.registration_end = activity.registration_end ? new Date(activity.registration_end) : null
  activityForm.max_participants = activity.max_participants || 0
  dialogVisible.value = true
}
    ...
    if (valid) {
      ...
      try {
        ...
        if (isEdit.value && editId.value) {
          res = await api.put(`/api/activities/${editId.value}`, data)
        } else {
          res = await api.post('/api/activities', data)
        }
        
        if (res.code === 200) {
          ElMessage.success(isEdit.value ? '活动更新成功' : '活动发布成功，等待审核')
...
const handleReview = async (activity, status) => {
  const action = status === 1 ? '通过' : '拒绝'
  try {
    ElMessageBox.confirm(`确定要${action}"${activity.title}"吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      const res = await api.put(`/api/activities/${activity.id}/review`, { status })
      if (res.code === 200) {
